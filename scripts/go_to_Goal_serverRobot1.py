...
robotname= "robot1" #Change this value 
...
def goToGoal(goal_x, goal_y, tf_listener, move_cmd, cmd_vel, position):
    autonomous_robots.msg.goToGoalActionFeedback= 'Starting'
    (position, rotation) = get_odom(tf_listener)
    last_rotation = 0
    linear_speed = 1
    angular_speed = 1
    ...
    while distance > 0.05:
        autonomous_robots.msg.goToGoalActionFeedback= 'Iam Going Now Please wait'
        (position, rotation) = get_odom(tf_listener)
        x_start = position.x
        y_start = position.y
    ...
    autonomous_robots.msg.goToGoalActionResult= 'DONE'
    rospy.loginfo("Stopping the robot...")
    cmd_vel.publish(Twist())
